...
export async function generateMetadata({ params }: FanpagePageProps): Promise<Metadata> {
  ...
  const seoContext = await getTenantSeoContext({ locale, slug: organization.slug, organizationType: "APPOINTMENT", subPath: "/fanpage" });
  return buildPublicMetadata({
    ...
    type: "article",
    keywords: ["Bazarbaaz", "fanpage", "appointment", organization.slug],
        ...
        <div className="container mx-auto px-4 py-10">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <Button variant="outline">
              <Link href={appointmentPath()}>{t("navigation.profile")}</Link>
            </Button>
          </div>
        </div>
      </section>
