...
export default function ServicesPage({ 
  ...
  useEffect(() => {
    ...
    appFetch(`/api/public/organizations/${slug}/services`)
      .then(res => {
...
<Link 
        ...
        {data.categories.length > 0 && (
          <div className="mb-8 flex flex-wrap gap-2">
            <Link
              href={appointmentRoutes.href("/services")}
            ...
            {data.categories.map((category) => (
              <Link
                ...
                href={appointmentRoutes.href(`/services/category/${category.slug || category.id}`)}
                className={cn(buttonVariants({ variant: "outline", size: "sm" }))}
              >
                {category.name} ({category.serviceCount})
              </Link>
              ...
              <Card key={locale+service.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                {getServicePrimaryMediaUrl(service) && (
                  <div className="h-48 overflow-hidden">
                    <Link href={appointmentRoutes.href(`/services/${service.slug || service.id}`)}>
                      <img
                ...
                <CardHeader>
                  <div className="flex items-start justify-between gap-2">
                    <CardTitle className="text-lg">
                      <Link href={appointmentRoutes.href(`/services/${service.slug || service.id}`)} className="hover:text-primary">
                        {service.name}
                      </Link>
                    </CardTitle>
                ...
                <CardContent>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link href={appointmentRoutes.href(`/booking?service=${service.id}`)}>
                      <Button>
