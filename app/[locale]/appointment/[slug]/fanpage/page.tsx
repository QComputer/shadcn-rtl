...
export async function generateMetadata({ params }: FanpagePageProps): Promise<Metadata> {
  ...
  const organization = await prisma.organization.findFirst({
    where: { slug, type: "APPOINTMENT", isActive: true, deletedAt: null },
  ...
  return buildPublicMetadata({
    locale,
    path: `/${locale}/appointment/${organization.slug}/fanpage`,
    ...
    type: "article",
    keywords: ["Bazar Baz", "fanpage", "appointment", organization.slug],
    alternatePath: (nextLocale) => `/${nextLocale}/appointment/${organization.slug}/fanpage`,
  });
}
    ...
    <div className="min-h-screen bg-muted/20">
      <JsonLd
        data={{
          ...
          "@type": "CollectionPage",
          "@id": `${getCanonicalUrl(`/${locale}/appointment/${organization.slug}/fanpage`)}#fanpage`,
          name: `${organization.name} fanpage`,
          description: truncateSeoText(organization.description, `Latest updates from ${organization.name}.`),
          url: getCanonicalUrl(`/${locale}/appointment/${organization.slug}/fanpage`),
        }}
      />
