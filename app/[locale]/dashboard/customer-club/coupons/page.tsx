...
export default function CouponsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const fetchCoupons = useCallback(async (signal?: AbortSignal) => {
    ...
    const response = await appFetch(`/api/dashboard/customer-club/coupons?organizationId=${encodeURIComponent(orgId)}`, {
      cache: "no-store",
      signal,
    })
  ...
  const createCoupon = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setNotice(null)
    try {
      const response = await appFetch("/api/dashboard/customer-club/coupons", {
        method: "POST",
