...
export default function WebPushDashboardPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const fetchPush = useCallback(async (signal?: AbortSignal) => {
    ...
    const response = await fetch(`/api/dashboard/customer-club/push?organizationId=${encodeURIComponent(orgId)}`, {
      cache: "no-store",
      signal,
    })
  ...
  const previewSend = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/dashboard/customer-club/push", {
        method: "POST",
