...
export default function ProductCategoriesPage({ 
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all([
      appFetch("/api/product-categories?pageSize=100")
        .then(res => {
  ...
  const getCategoryPreviewPath = () => {
    ...
    return segment && organizationSlug ? buildOrganizationPublicPath({ locale, organizationSlug, surface: "shop", subPath: `/category/${segment}` }) : null
  }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const response = await appFetch(`/api/product-categories/${categoryToDelete.id}`, {
        method: "DELETE",
      })
