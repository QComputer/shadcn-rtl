...
The current working baseline after P41 overlays is source-validator green.
...
```txt
P42 — dashboard server/API guard parity review
```

Scope:

1. Audit server components and API handlers behind dashboard surfaces for route-policy parity.
2. Confirm page-local server/API guards still enforce data access independently of the client boundary.
3. Add focused static checks for high-risk dashboard data routes.
