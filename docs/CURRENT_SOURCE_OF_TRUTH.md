...
The current working baseline after P118 Creative Studio organization-brand provider execution implementation is source-validator green after the P118 validation gate.

P118 adds server-mediated organization-brand provider execution wiring behind the P117 rollout gate. Logo and cover generation requests now use the AI Media Service organization-brand endpoint only when rollout and execution flags are explicitly enabled and dry-run is off; default behavior remains disabled/dry-run, generated assets stay draft-only, and public apply still requires selected-candidate review and confirmation.
