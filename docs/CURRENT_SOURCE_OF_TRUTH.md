...
The current working baseline after P115 Creative Studio organization logo and cover generation request controls is source-validator green after the P115 validation gate.

P115 adds request-only Creative Studio controls for organization `LOGO` and `COVER` work on top of the P114 readiness plan. The dashboard can record internal logo/cover request jobs with P115 metadata, while provider execution remains disabled, generated assets stay draft-first, and public logo/cover apply still requires a public asset URL, selected-candidate review, and the existing confirmation-gated apply action.
