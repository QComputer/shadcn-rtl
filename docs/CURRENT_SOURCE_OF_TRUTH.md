...
The current working baseline after P113 Creative Studio generated-asset selection polish and deployed acceptance is source-validator green after the P113 validation gate.

P113 adds explicit generated-asset selection polish on top of the P112 product-image generation workflow. Sellers can mark one generated candidate per Creative Studio job as selected, the system records `ASSET_SELECTED` usage and audit evidence, and deployed acceptance checks verify auth boundaries plus secret-safe status/job/usage reads. Selection remains internal and does not mutate public pages; public changes still require the existing confirmation-gated apply controls.
