...
The current working baseline after P116 Creative Studio organization logo and cover generated-asset acceptance is source-validator green after the P116 validation gate.

P116 hardens the organization `LOGO` and `COVER` generated-asset acceptance path on top of the P115 request-only controls. Organization brand public apply now requires a public asset URL, selected-candidate review, target-field match, and the existing confirmation-gated apply action; provider execution remains disabled until a future rollout gate.
