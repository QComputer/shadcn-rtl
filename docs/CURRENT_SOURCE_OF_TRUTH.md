...
The current working baseline after P117 Creative Studio organization-brand provider execution rollout gate is source-validator green after the P117 validation gate.

P117 adds an explicit organization-brand provider execution rollout gate on top of the P116 acceptance path. Organization logo/cover provider execution now has server-only requested/configured/approval/rollback status, runtime env validation, safe admin visibility, and deployed smoke coverage, while public apply still requires selected-candidate review and confirmation.
