/*
  ...
  - A unique constraint covering the columns `[prerparationProgressId]` on the table `Order` will be added. If there are existing duplicate values, this will fail.
...
ADD COLUMN     "prerparationProgressId" TEXT;
...
-- CreateIndex
CREATE UNIQUE INDEX "Order_prerparationProgressId_key" ON "Order"("prerparationProgressId");
...
-- AddForeignKey
ALTER TABLE "Order" ADD CONSTRAINT "Order_prerparationProgressId_fkey" FOREIGN KEY ("prerparationProgressId") REFERENCES "Progress"("id") ON DELETE SET NULL ON UPDATE CASCADE;
