...
function createDockerDatabase() {
  containerName = `bazar-baz-ai-media-assets-${stamp}`;
  const databaseName = `bazar_baz_ai_media_assets_${stamp}`;
...
function startContractMock() {
  ...
  contractMockPort = process.env.AI_MEDIA_CONTRACT_MOCK_PORT || "4767";
...
function buildEnv(databaseUrl, mockUrl) {
  ...
  return {
    ...
    AI_MEDIA_LOCAL_STORAGE_ROOT: path.join(process.cwd(), ".tmp", "ai-media-asset-consumption", "storage"),
    SMS_DRY_RUN: "true",
    EMAIL_DRY_RUN: "true",
...
try {
  ...
  runPnpm(["exec", "tsx", "scripts/e2e/ai-media-local-docker-asset-consumption-e2e.mts"], env);

  console.log("AI media local Docker asset consumption E2E passed.");
} catch (error) {
  ...
  try { fs.rmSync(path.join(process.cwd(), ".tmp", "ai-media-asset-consumption"), { recursive: true, force: true }); } catch { /* ignore */ }
}
