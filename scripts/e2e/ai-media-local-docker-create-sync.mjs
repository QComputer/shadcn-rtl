...
function createDockerDatabase() {
  containerName = `bazar-baz-ai-media-createsync-${stamp}`;
  const databaseName = `bazar_baz_ai_media_createsync_${stamp}`;
...
function startContractMock() {
  ...
  contractMockPort = process.env.AI_MEDIA_CONTRACT_MOCK_PORT || "4766";
...
try {
  ...
  try { fs.rmSync(path.join(process.cwd(), ".tmp", "ai-media-create-sync"), { recursive: true, force: true }); } catch { /* ignore */ }
}
