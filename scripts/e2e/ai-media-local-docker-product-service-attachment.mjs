...
function createDockerDatabase() {
  containerName = `bazar-baz-ai-media-attach-${stamp}`;
  const databaseName = `bazar_baz_ai_media_attach_${stamp}`;
  capture("docker", [
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    if (ready.status === 0) {
      return { databaseUrl: `postgresql://postgres:postgres@127.0.0.1:${port}/${databaseName}?schema=public` };
    }
    sleep(500);
  }
...
function startContractMock() {
  ...
  const port = process.env.AI_MEDIA_CONTRACT_MOCK_PORT || "4768";
  contractMockProcess = spawn(process.execPath, ["scripts/ai-media/local-contract-mock.mjs"], {
    env: { ...process.env, AI_MEDIA_CONTRACT_MOCK_PORT: port, AI_MEDIA_SERVICE_INTERNAL_KEY: process.env.AI_MEDIA_SERVICE_INTERNAL_KEY },
    stdio: "ignore",
    detached: true,
  });
...
function stopContractMock() {
  if (contractMockProcess?.pid) {
    try { process.kill(-contractMockProcess.pid); } catch {}
  }
}

function buildEnv(databaseUrl, mockUrl) {
  if (!/127\.0\.0\.1|localhost/.test(databaseUrl) || /neon/i.test(databaseUrl)) {
    throw new Error("Refusing to run product/service attachment E2E against non-local database.");
  }
  return {
    ...process.env,
    ...
    AI_MEDIA_LOCAL_STORAGE_ROOT: path.join(process.cwd(), ".tmp", "ai-media-product-service-attachment", "storage"),
    SMS_DRY_RUN: "true",
    EMAIL_DRY_RUN: "true",
...
try {
  console.log("LOCAL_DOCKER_MOCK_E2E product/service attachment gate starting.");
  const database = createDockerDatabase();
  ...
  runPnpm(["exec", "tsx", "scripts/e2e/ai-media-local-docker-product-service-attachment-e2e.mts"], env);

  console.log("AI media local Docker product/service attachment E2E passed.");
} catch (error) {
  ...
  try { fs.rmSync(path.join(process.cwd(), ".tmp", "ai-media-product-service-attachment"), { recursive: true, force: true }); } catch {}
}
