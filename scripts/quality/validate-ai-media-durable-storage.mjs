...
add("remote AI image copy normalizes content type", /function normalizeImageContentType/.test(mediaStorage) && /contentType\.split\(";"\)/.test(mediaStorage))
add("remote AI image copy rejects oversized content-length", /content-length/.test(mediaStorage) && /MAX_IMAGE_UPLOAD_BYTES/.test(mediaStorage) && /Remote image size is invalid/.test(mediaStorage))
add("remote AI image copy validates bytes before Blob write", /validateImageBuffer\(buffer, contentType, buffer\.length\)/.test(mediaStorage))
add("remote AI image copy stores public Blob image", /writeStoredImage\(filename, buffer, "PUBLIC"\)/.test(mediaStorage))
add("selected AI image storage status type exists", /type AiSelectedImageStorageStatus = "blob" \| "remote-unconfigured" \| "remote-fallback"/.test(service))
add("selected AI image reports blob success", /storageStatus = "blob"/.test(service) && /storedDurably = true/.test(service))
add("selected AI image reports remote fallback on Blob copy failure", /storageStatus = "remote-fallback"/.test(service) && /falling back to remote URL/.test(service))
add("selected AI image reports unconfigured remote URL fallback", /storageStatus: AiSelectedImageStorageStatus = "remote-unconfigured"/.test(service) && /BLOB_READ_WRITE_TOKEN is not configured/.test(service))
