...
add("edit product AI status fetch waits for dashboard access", /if \(!hasAccess \|\| accessLoading\) return/.test(editPage) && /setAiFeatureEnabled\(Boolean\((data|statusData)\.enabled\)\)/.test(editPage))
add("new product AI disabled hint waits for dashboard access", /if \(!hasAccess \|\| accessLoading\) return/.test(newPage) && /setAiFeatureEnabled\(Boolean\((data|statusData)\.enabled\)\)/.test(newPage))
add("edit product AI job creation is feature-gated", /if \(!aiFeatureEnabled(?: \|\| !aiCanCreate)?\)/.test(editPage) && /پیشنهاد تصویر AI در حال حاضر فعال نیست/.test(editPage))
add("edit product AI polling accepts outputs and output_images", (/Array\.isArray\(data\.job\?\.outputs\)/.test(editPage) && /Array\.isArray\(data\.job\?\.output_images\)/.test(editPage)) || (/normalizeAiOutputs/.test(editPage) && /Array\.isArray\(job\.outputs\)/.test(editPage) && /"output_images" in job/.test(editPage)))
