...
const checks: Array<[string, boolean]> = [
  ...
  ["DB identity guard supports accepted-risk non-isolated mode", /AI_MEDIA_PREVIEW_DB_NON_ISOLATED_WRITE_ACCEPTED/.test(dbGuard) && /ACCEPTED_RISK_NON_ISOLATED_DB/.test(dbGuard)],
  ["DB identity guard treats identical Preview URLs as warning", /identical inside this environment/.test(dbGuard) && /warnings/.test(dbGuard) && !/must not be identical for Preview write proof/.test(dbGuard)],
  ...
  ["E2E test script exists and covers DB guard route and service", /preview-mock-write-e2e/.test(packageJson) && /Preview DB identity/.test(tests) && /accepted-risk non-isolated MOCK E2E/.test(tests) && /submitPreviewMockAiMediaJob/.test(tests)],
