...
add("deployed smoke blocks unauthenticated protected routes", /unauthenticated AI job creation is blocked/i.test(deployedSmoke) && /unauthenticated AI image select is blocked/i.test(deployedSmoke))
...
add("package exposes AI media validators", /"quality:ai-media":\s*"node scripts\/quality\/validate-ai-media\.mjs"/.test(packageJson) && /"quality:ai-media-health-gate"/.test(packageJson) && /"quality:ai-media-mock-flow"/.test(packageJson) && /"quality:ai-media-durable-storage"/.test(packageJson) && /"quality:ai-media-long-running-ux"/.test(packageJson) && /"quality:ai-media-usage-controls"/.test(packageJson) && /"quality:import-ai-media-bridge"/.test(packageJson) && /"quality:ai-media-client"/.test(packageJson) && /"quality:ai-media-mock"/.test(packageJson) && /"quality:ai-media-deployed-smoke"/.test(packageJson) && /"smoke:deployed:ai-media"/.test(packageJson))
