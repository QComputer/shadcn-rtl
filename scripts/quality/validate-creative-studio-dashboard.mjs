...
add("dashboard is read-only", !/method:\s*["']POST["']/.test(page) && !/\/api\/dashboard\/creative-studio\/assets\/\$\{[^}]+}\}\/apply/.test(page) && !/recordAssetApplication/.test(page))
add("dashboard has Persian-first Creative Studio copy", /استودیوی خلاقیت/.test(page) && /تغییر عمومی/.test(page) && /فقط پیش‌نویس/.test(page))
...
add("roadmap marks P109 complete", /Completed through \*\*P109 - Creative Studio dashboard shell and read-only job review\*\*/.test(roadmap) && /\| P109 \| Creative Studio dashboard shell and read-only job review\. \|/.test(roadmap) && /\| P110 \| Creative Studio apply controls and cache-safe public asset updates\. \|/.test(roadmap))
add("source of truth names P109 baseline", /after P109 Creative Studio dashboard shell and read-only job review/.test(sourceOfTruth) && /Creative Studio dashboard review exists/.test(sourceOfTruth))
