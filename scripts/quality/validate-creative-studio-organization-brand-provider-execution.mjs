...
add("server-only AI media client fails closed for unavailable organization brand execution", /import "server-only"/.test(client) && /createOrganizationBrandGenerationJob/.test(client) && /CAPABILITY_UNAVAILABLE/.test(client) && !/fetch\(`?\$\{config\.url\}\/v1\/organization-brand/.test(client));
...
add("execution gate requires rollout plus compatible live contract", /CREATIVE_STUDIO_ORGANIZATION_BRAND_PROVIDER_EXECUTION_ENABLED/.test(gate) && /CREATIVE_STUDIO_ORGANIZATION_BRAND_PROVIDER_DRY_RUN/.test(gate) && /liveContractSupportsBrand/.test(gate) && /live-contract-unsupported/.test(gate));
...
add("service implements P118 provider execution path behind fail-closed gate", /requestOrganizationBrandProviderExecution/.test(service) && /canCallProvider/.test(service) && /providerExecutionEnabled && !requestDryRun/.test(service) && /unavailable in the live AI media contract/.test(service));
add("service stores provider metadata without fabricating remote job id", /p118OrganizationBrandProviderExecution/.test(service) && /executionMode/.test(service) && /getRemoteOrganizationBrandJobIdFromInputs/.test(service));
...
add("README preserves completed P118/P119 baseline", /P118 Creative Studio organization-brand provider execution/.test(readme) && /P119 Creative Studio provider result ingestion/.test(readme));
add("roadmap preserves P118/P119 while tracking current AI media milestone", /BB-AI-MEDIA-ONLINE-MILESTONE-01/.test(roadmap) && /\| P118 \| Creative Studio organization-brand provider execution implementation\. \|/.test(roadmap) && /\| P119 \| Creative Studio provider result ingestion and review stabilization\. \|/.test(roadmap) && /\| P120 \| Creative Studio reviewed asset apply and rollback workflow\. \|/.test(roadmap));
add("source of truth names P119 baseline and current AI media milestone", /BB-AI-MEDIA-ONLINE-MILESTONE-01/.test(sourceOfTruth) && /Creative Studio organization-brand provider execution wiring exists/.test(sourceOfTruth) && /Creative Studio provider result ingestion exists/.test(sourceOfTruth));
