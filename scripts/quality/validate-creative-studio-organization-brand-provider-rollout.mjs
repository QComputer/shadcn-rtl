...
add("readiness exposes P117 rollout gate without public auto-apply", /organizationBrandProvider/.test(readiness) && /providerExecutionGatePhase:\s*"P117"/.test(readiness) && /rolloutGate:\s*organizationBrandProvider/.test(readiness) && /publicAutoApplyAllowed:\s*false/.test(readiness) && /nextPhase:\s*"P120E — SMS delivery reports and provider reconciliation"/.test(readiness));
...
add("README marks current P119 baseline and P120 next", /Latest completed implementation phase:\s+\*\*P120D - SMS.ir provider completion\*\*/.test(readme) && /Recommended next phase:\s+\*\*P120E — SMS delivery reports and provider reconciliation\*\*/.test(readme));
add("roadmap keeps P117 complete through P119 baseline", /Completed through \*\*P120D - SMS.ir provider completion\*\*/.test(roadmap) && /\| P117 \| Creative Studio organization-brand provider execution rollout gate\. \|/.test(roadmap) && /\| P118 \| Creative Studio organization-brand provider execution implementation\. \|/.test(roadmap) && /\| P119 \| Creative Studio provider result ingestion and review stabilization\. \|/.test(roadmap));
