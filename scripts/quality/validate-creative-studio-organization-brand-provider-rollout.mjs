...
add("readiness exposes P117 rollout gate without public auto-apply", /organizationBrandProvider/.test(readiness) && /providerExecutionGatePhase:\s*"P117"/.test(readiness) && /rolloutGate:\s*organizationBrandProvider/.test(readiness) && /publicAutoApplyAllowed:\s*false/.test(readiness) && /nextPhase:\s*"P118 - Creative Studio organization-brand provider execution implementation"/.test(readiness));
...
add("service records P117 gate metadata but does not execute provider", /getOrganizationBrandProviderStatus/.test(service) && /p117OrganizationBrandProviderGate/.test(service) && /providerExecutionGateOnly:\s*true/.test(service) && /providerExecutionReadyButNotExecuted/.test(service) && /providerExecutionEnabled:\s*false/.test(service) && !/createOrganizationBrandJob/.test(service) && !/fetch\([^)]*organization-brand/.test(service));
...
add("dashboard posts request-only P117 metadata", /p117OrganizationBrandProviderGate/.test(page) && /providerExecutionGateOnly:\s*true/.test(page) && /providerExecutionEnabled:\s*false/.test(page) && !/\/v1\/organization-brand\/jobs/.test(page));
...
add("deployed smoke verifies P117 rollout gate and request-only behavior", /providerExecutionGatePhase === "P117"/.test(smoke) && /p117DeployedSmoke/.test(smoke) && /p117OrganizationBrandProviderGate/.test(smoke) && /brand rollout gate must not call a real provider/.test(smoke));
...
add("README marks P117 complete and P118 next", /Latest completed implementation phase:\s+\*\*P117 - Creative Studio organization-brand provider execution rollout gate\*\*/.test(readme) && /Recommended next phase:\s+\*\*P118 - Creative Studio organization-brand provider execution implementation\*\*/.test(readme));
add("roadmap marks P117 complete and P118 next", /Completed through \*\*P117 - Creative Studio organization-brand provider execution rollout gate\*\*/.test(roadmap) && /\| P117 \| Creative Studio organization-brand provider execution rollout gate\. \|/.test(roadmap) && /\| P118 \| Creative Studio organization-brand provider execution implementation\. \|/.test(roadmap));
add("source of truth names P117 baseline", /after P117 Creative Studio organization-brand provider execution rollout gate/.test(sourceOfTruth) && /Creative Studio organization-brand provider execution rollout gate exists/.test(sourceOfTruth));
