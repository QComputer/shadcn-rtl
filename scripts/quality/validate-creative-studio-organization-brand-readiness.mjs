...
add("README marks P114 complete and P115 next", /Latest completed implementation phase:\s+\*\*P120B - Customer order lifecycle notifications and guest SMS dry-run review\*\*/.test(readme) && /Recommended next phase:\s+\*\*P120C - Notification delivery observability and retry review\*\*/.test(readme));
add("roadmap marks P114 complete and P115 next", /Completed through \*\*P120B - Customer order lifecycle notifications and guest SMS dry-run review\*\*/.test(roadmap) && /\| P114 \| Creative Studio organization-brand generation planning and readiness gate\. \|/.test(roadmap) && /\| P115 \| Creative Studio organization logo and cover generation request controls\. \|/.test(roadmap));
add("source of truth names P114 baseline", /after P119 Creative Studio provider result ingestion and review stabilization/.test(sourceOfTruth) && /Creative Studio organization-brand generation readiness exists/.test(sourceOfTruth));
