...
add("readiness helper exposes P115 request controls separately from provider execution", /requestControlsPhase:\s*"P115"/.test(readiness) && /requestControlsEnabled:\s*true/.test(readiness) && /providerExecutionEnabled:\s*false/.test(readiness) && /requestOnlyJobPersistence:\s*true/.test(readiness));
add("readiness keeps organization brand provider execution disabled", /generationRequestEnabled:\s*false/.test(readiness) && /generationUiEnabled:\s*false/.test(readiness) && /providerContractReady:\s*false/.test(readiness) && /Organization logo\/cover provider execution remains disabled/.test(readiness));
...
add("README marks P115 complete and P116 next", /Latest completed implementation phase:\s+\*\*P115 - Creative Studio organization logo and cover generation request controls\*\*/.test(readme) && /Recommended next phase:\s+\*\*P116 - Creative Studio organization logo and cover generated-asset acceptance\*\*/.test(readme));
add("roadmap marks P115 complete and P116 next", /Completed through \*\*P115 - Creative Studio organization logo and cover generation request controls\*\*/.test(roadmap) && /\| P115 \| Creative Studio organization logo and cover generation request controls\. \|/.test(roadmap) && /\| P116 \| Creative Studio organization logo and cover generated-asset acceptance\. \|/.test(roadmap));
add("source of truth names P115 baseline", /after P115 Creative Studio organization logo and cover generation request controls/.test(sourceOfTruth) && /Creative Studio organization logo and cover request controls exist/.test(sourceOfTruth));
