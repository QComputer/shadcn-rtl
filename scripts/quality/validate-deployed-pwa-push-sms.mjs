...
add("README keeps P104 complete while marking P108 latest", /P104 deployed PWA\/Push\/SMS smoke gates/.test(readme) && /Latest completed implementation phase:\s+\*\*P108 - Creative Studio server foundation\*\*/.test(readme) && /P109 - Creative Studio dashboard shell and read-only job review/.test(readme))
add("roadmap marks P104 complete in P108 progression", /\| P104 \| Deployed PWA, Push, and SMS smoke gates\. \|/.test(roadmap) && /Completed through \*\*P108 - Creative Studio server foundation\*\*/.test(roadmap))
add("source of truth names P108 baseline while keeping P104 smoke", /after P108 Creative Studio server foundation/.test(sourceOfTruth) && /Deployed PWA, Push, and SMS smoke exists/.test(sourceOfTruth))
