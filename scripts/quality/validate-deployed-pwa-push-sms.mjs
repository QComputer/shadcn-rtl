...
add("README keeps P104 complete while marking P109 latest", /P104 deployed PWA\/Push\/SMS smoke gates/.test(readme) && /Latest completed implementation phase:\s+\*\*P120 - Creative Studio reviewed asset apply and rollback workflow\*\*/.test(readme) && /P120B - Customer order lifecycle notifications and guest SMS dry-run review/.test(readme))
add("roadmap marks P104 complete in P109 progression", /\| P104 \| Deployed PWA, Push, and SMS smoke gates\. \|/.test(roadmap) && /Completed through \*\*P120 - Creative Studio reviewed asset apply and rollback workflow\*\*/.test(roadmap))
add("source of truth names P109 baseline while keeping P104 smoke", /after P119 Creative Studio provider result ingestion and review stabilization/.test(sourceOfTruth) && /Deployed PWA, Push, and SMS smoke exists/.test(sourceOfTruth))
