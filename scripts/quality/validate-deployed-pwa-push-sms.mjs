...
add("README keeps P104 complete while marking P109 latest", /P104 deployed PWA\/Push\/SMS smoke gates/.test(readme) && /Latest completed implementation phase:\s+\*\*P110 - Creative Studio apply controls and cache-safe public asset updates\*\*/.test(readme) && /P111 - Creative Studio generation readiness gate and AI-service contract sync/.test(readme))
add("roadmap marks P104 complete in P109 progression", /\| P104 \| Deployed PWA, Push, and SMS smoke gates\. \|/.test(roadmap) && /Completed through \*\*P110 - Creative Studio apply controls and cache-safe public asset updates\*\*/.test(roadmap))
add("source of truth names P109 baseline while keeping P104 smoke", /after P110 Creative Studio apply controls and cache-safe public asset updates/.test(sourceOfTruth) && /Deployed PWA, Push, and SMS smoke exists/.test(sourceOfTruth))
