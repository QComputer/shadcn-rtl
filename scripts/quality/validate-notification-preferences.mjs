...
add("README keeps P99 complete while marking P102 latest and P103 next", /\| 99 \| Notification domain model and preferences/.test(readme) && /Latest completed implementation phase:\s+\*\*P102 - Notification templates, routing, and delivery policies\*\*/.test(readme) && /Recommended next phase:\s+\*\*P103 - Admin\/operator notification dashboard\*\*/.test(readme))
add("roadmap marks P99 complete in P102 progression", /\| P99 \| Notification domain model and preferences\. \|/.test(roadmap) && /Completed through \*\*P102 - Notification templates, routing, and delivery policies\*\*/.test(roadmap))
add("source of truth names P102 baseline", /after P102 Notification templates, routing, and delivery policies/.test(sourceOfTruth) && /Notification domain model and preferences exists/.test(sourceOfTruth))
