...
add("README keeps P99 complete while marking P109 latest", /\| 99 \| Notification domain model and preferences/.test(readme) && /Latest completed implementation phase:\s+\*\*P120 - Creative Studio reviewed asset apply and rollback workflow\*\*/.test(readme) && /P120B - Customer order lifecycle notifications and guest SMS dry-run review/.test(readme))
add("roadmap marks P99 complete in P109 progression", /\| P99 \| Notification domain model and preferences\. \|/.test(roadmap) && /Completed through \*\*P120 - Creative Studio reviewed asset apply and rollback workflow\*\*/.test(roadmap))
add("source of truth names P109 baseline", /after P119 Creative Studio provider result ingestion and review stabilization/.test(sourceOfTruth) && /Notification domain model and preferences exists/.test(sourceOfTruth))
