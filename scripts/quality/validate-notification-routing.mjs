...
add("README keeps P102 complete while marking P109 latest", /\| 102 \| Notification templates, routing, and delivery policies/.test(readme) && /Latest completed implementation phase:\s+\*\*P111 - Creative Studio generation readiness gate and AI-service contract sync\*\*/.test(readme) && /P112 - Creative Studio product-image generation request controls and long-running job UX/.test(readme))
add("roadmap marks P102 complete in P109 progression", /\| P102 \| Notification templates, routing, and delivery policies\. \|/.test(roadmap) && /Completed through \*\*P111 - Creative Studio generation readiness gate and AI-service contract sync\*\*/.test(roadmap))
add("source of truth names P109 baseline", /after P111 Creative Studio generation readiness gate and AI-service contract sync/.test(sourceOfTruth) && /Notification routing exists/.test(sourceOfTruth))
