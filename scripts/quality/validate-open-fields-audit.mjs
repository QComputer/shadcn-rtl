...
add("README keeps P96 complete while marking P106 latest", /\| 96 \| Open Fields and Workflow Completion Audit/.test(readme) && /Latest completed implementation phase:\s+\*\*P106 - PWA\/Push\/SMS source acceptance and secretless packaging gate\*\*/.test(readme) && /P107 - Creative Studio integration planning/.test(readme))
add("roadmap keeps P96 complete while marking P106 baseline", /\| P96 \| Open fields and workflow completion audit\. \|/.test(roadmap) && /Completed through \*\*P106 - PWA\/Push\/SMS source acceptance and secretless packaging gate\*\*/.test(roadmap))
add("source of truth keeps P96 audit while naming P106 baseline", /Open Fields and Workflow Completion Audit exists/.test(sourceOfTruth) && /after P106 PWA\/Push\/SMS source acceptance/.test(sourceOfTruth))
