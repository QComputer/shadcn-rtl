...
add("README keeps P105 complete while marking P109 latest", /\| 105 \| Production rollout runbook/.test(readme) && /Latest completed implementation phase:\s+\*\*P112 - Creative Studio product-image generation request controls and long-running job UX\*\*/.test(readme) && /P113 - Creative Studio generated-asset selection polish and deployed acceptance/.test(readme))
add("roadmap keeps P105 complete in P109 progression", /\| P105 \| Production rollout runbook\. \|/.test(roadmap) && /Completed through \*\*P112 - Creative Studio product-image generation request controls and long-running job UX\*\*/.test(roadmap))
add("source of truth names P109 baseline while keeping P105 runbook", /after P112 Creative Studio product-image generation request controls and long-running job UX/.test(sourceOfTruth) && /Production rollout runbook exists/.test(sourceOfTruth))
