...
add("README keeps P105 complete while marking P109 latest", /\| 105 \| Production rollout runbook/.test(readme) && /Latest completed implementation phase:\s+\*\*P109 - Creative Studio dashboard shell and read-only job review\*\*/.test(readme) && /P110 - Creative Studio apply controls and cache-safe public asset updates/.test(readme))
add("roadmap keeps P105 complete in P109 progression", /\| P105 \| Production rollout runbook\. \|/.test(roadmap) && /Completed through \*\*P109 - Creative Studio dashboard shell and read-only job review\*\*/.test(roadmap))
add("source of truth names P109 baseline while keeping P105 runbook", /after P109 Creative Studio dashboard shell and read-only job review/.test(sourceOfTruth) && /Production rollout runbook exists/.test(sourceOfTruth))
