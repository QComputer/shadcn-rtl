...
add("README keeps P105 complete while marking P108 latest", /\| 105 \| Production rollout runbook/.test(readme) && /Latest completed implementation phase:\s+\*\*P108 - Creative Studio server foundation\*\*/.test(readme) && /P109 - Creative Studio dashboard shell and read-only job review/.test(readme))
add("roadmap keeps P105 complete in P108 progression", /\| P105 \| Production rollout runbook\. \|/.test(roadmap) && /Completed through \*\*P108 - Creative Studio server foundation\*\*/.test(roadmap))
add("source of truth names P108 baseline while keeping P105 runbook", /after P108 Creative Studio server foundation/.test(sourceOfTruth) && /Production rollout runbook exists/.test(sourceOfTruth))
