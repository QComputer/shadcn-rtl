...
add("README keeps P98 complete while marking P109 latest", /\| 98 \| Offline shell, caching, and PWA quality gates/.test(readme) && /Latest completed implementation phase:\s+\*\*P111 - Creative Studio generation readiness gate and AI-service contract sync\*\*/.test(readme) && /P112 - Creative Studio product-image generation request controls and long-running job UX/.test(readme))
add("roadmap marks P98 complete in P109 progression", /\| P98 \| Offline shell, caching, and PWA quality gates\. \|/.test(roadmap) && /Completed through \*\*P111 - Creative Studio generation readiness gate and AI-service contract sync\*\*/.test(roadmap))
add("source of truth names P109 baseline while keeping P98 offline shell", /Offline Shell, Caching, and PWA Quality Gates exist/.test(sourceOfTruth) && /after P111 Creative Studio generation readiness gate and AI-service contract sync/.test(sourceOfTruth))
