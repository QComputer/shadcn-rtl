...
add("README keeps P95 source cleanup in current progression", /\| 95 \| Source Cleanup and Current-State Verification/.test(readme) && /Latest completed implementation phase:\s+\*\*P114 - Creative Studio organization-brand generation planning and readiness gate\*\*/.test(readme))
add("README recommends P108 after P107", /Recommended next phase:\s+\*\*P115 - Creative Studio organization logo and cover generation request controls\*\*/.test(readme))
add("roadmap keeps P95 complete in P109 progression", /\| P95 \| Source cleanup and current-state verification\. \|/.test(roadmap) && /Completed through \*\*P114 - Creative Studio organization-brand generation planning and readiness gate\*\*/.test(roadmap))
add("source of truth keeps P95 source cleanup while naming P109 baseline", /Source Cleanup and Current-State Verification exists/.test(sourceOfTruth) && /after P114 Creative Studio organization-brand generation planning and readiness gate/.test(sourceOfTruth))
