...
add("README keeps P95 source cleanup in current progression", /\| 95 \| Source Cleanup and Current-State Verification/.test(readme) && /Latest completed implementation phase:\s+\*\*P107 - Creative Studio integration planning for main Bazar Baz\*\*/.test(readme))
add("README recommends P108 after P107", /Recommended next phase:\s+\*\*P108 - Creative Studio server foundation\*\*/.test(readme))
add("roadmap keeps P95 complete in P107 progression", /\| P95 \| Source cleanup and current-state verification\. \|/.test(roadmap) && /Completed through \*\*P107 - Creative Studio integration planning for main Bazar Baz\*\*/.test(roadmap))
add("source of truth keeps P95 source cleanup while naming P107 baseline", /Source Cleanup and Current-State Verification exists/.test(sourceOfTruth) && /after P107 Creative Studio integration planning/.test(sourceOfTruth))
