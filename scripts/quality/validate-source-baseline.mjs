...
add("README keeps P95 source cleanup in current progression", /\| 95 \| Source Cleanup and Current-State Verification/.test(readme) && /Latest completed implementation phase:\s+\*\*P101 - SMS provider abstraction and sms.ir integration\*\*/.test(readme))
add("README recommends P102 after P101", /Recommended next phase:\s+\*\*P102 - Notification templates, routing, and delivery policies\*\*/.test(readme))
add("roadmap keeps P95 complete in P101 progression", /\| P95 \| Source cleanup and current-state verification\. \|/.test(roadmap) && /Completed through \*\*P101 - SMS provider abstraction and sms.ir integration\*\*/.test(roadmap))
add("source of truth keeps P95 source cleanup while naming P101 baseline", /Source Cleanup and Current-State Verification exists/.test(sourceOfTruth) && /after P101 SMS provider abstraction and sms.ir integration/.test(sourceOfTruth))
...
for (const file of tracked) {
  ...
  for (const { name, pattern } of secretAssignmentPatterns) {
    ...
    if (value && !/^["']?["']?$/.test(value) && !/^replace-with/i.test(value)) {
