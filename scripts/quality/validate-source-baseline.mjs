...
add("README keeps P95 source cleanup in current progression", /\| 95 \| Source Cleanup and Current-State Verification/.test(readme) && /Latest completed implementation phase:\s+\*\*P97 - PWA foundation and install experience\*\*/.test(readme))
add("README recommends P98 after P97", /Recommended next phase:\s+\*\*P98 - Offline shell, caching, and PWA quality gates\*\*/.test(readme))
add("roadmap keeps P95 complete in P97 progression", /\| P95 \| Source cleanup and current-state verification\. \|/.test(roadmap) && /Completed through \*\*P97 - PWA foundation and install experience\*\*/.test(roadmap))
add("source of truth keeps P95 source cleanup while naming P97 baseline", /Source Cleanup and Current-State Verification exists/.test(sourceOfTruth) && /after P97 PWA foundation and install experience/.test(sourceOfTruth))
