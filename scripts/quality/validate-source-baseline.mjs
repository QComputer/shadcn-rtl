...
add("README keeps P95 source cleanup in current progression", /\| 95 \| Source Cleanup and Current-State Verification/.test(readme) && /Latest completed implementation phase:\s+\*\*P100 - Web Push notification service\*\*/.test(readme))
add("README recommends P101 after P100", /Recommended next phase:\s+\*\*P101 - SMS provider abstraction and sms.ir integration\*\*/.test(readme))
add("roadmap keeps P95 complete in P100 progression", /\| P95 \| Source cleanup and current-state verification\. \|/.test(roadmap) && /Completed through \*\*P100 - Web Push notification service\*\*/.test(roadmap))
add("source of truth keeps P95 source cleanup while naming P100 baseline", /Source Cleanup and Current-State Verification exists/.test(sourceOfTruth) && /after P100 Web Push notification service/.test(sourceOfTruth))
