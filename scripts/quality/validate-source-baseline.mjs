...
add("README keeps P95 source cleanup in current progression", /\| 95 \| Source Cleanup and Current-State Verification/.test(readme) && /Latest completed implementation phase:\s+\*\*P108 - Creative Studio server foundation\*\*/.test(readme))
add("README recommends P108 after P107", /Recommended next phase:\s+\*\*P109 - Creative Studio dashboard shell and read-only job review\*\*/.test(readme))
add("roadmap keeps P95 complete in P108 progression", /\| P95 \| Source cleanup and current-state verification\. \|/.test(roadmap) && /Completed through \*\*P108 - Creative Studio server foundation\*\*/.test(roadmap))
add("source of truth keeps P95 source cleanup while naming P108 baseline", /Source Cleanup and Current-State Verification exists/.test(sourceOfTruth) && /after P108 Creative Studio server foundation/.test(sourceOfTruth))
