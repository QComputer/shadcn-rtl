...
add("README keeps P100 complete while marking P109 latest", /\| 100 \| Web Push notification service/.test(readme) && /Latest completed implementation phase:\s+\*\*P111 - Creative Studio generation readiness gate and AI-service contract sync\*\*/.test(readme) && /P112 - Creative Studio product-image generation request controls and long-running job UX/.test(readme))
add("roadmap keeps P100 complete while marking P109 baseline", /\| P100 \| Web Push notification service\. \|/.test(roadmap) && /Completed through \*\*P111 - Creative Studio generation readiness gate and AI-service contract sync\*\*/.test(roadmap))
add("source of truth names P109 baseline and keeps P100 summary", /after P111 Creative Studio generation readiness gate and AI-service contract sync/.test(sourceOfTruth) && /Web Push notification service exists/.test(sourceOfTruth))
