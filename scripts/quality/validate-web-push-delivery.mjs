...
add("README keeps P100 complete while marking P109 latest", /\| 100 \| Web Push notification service/.test(readme) && /Latest completed implementation phase:\s+\*\*P120B - Customer order lifecycle notifications and guest SMS dry-run review\*\*/.test(readme) && /P120C - Notification delivery observability and retry review/.test(readme))
add("roadmap keeps P100 complete while marking P109 baseline", /\| P100 \| Web Push notification service\. \|/.test(roadmap) && /Completed through \*\*P120B - Customer order lifecycle notifications and guest SMS dry-run review\*\*/.test(roadmap))
add("source of truth names P109 baseline and keeps P100 summary", /after P119 Creative Studio provider result ingestion and review stabilization/.test(sourceOfTruth) && /Web Push notification service exists/.test(sourceOfTruth))
