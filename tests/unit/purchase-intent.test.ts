...
describe("stateless purchase intent", () => {
  it("builds an APP subdomain handoff from immutable product identity", () => {
    ...
    assert.deepEqual(result, {
      href: "https://app.tenant.example/purchase/product/product-stable-id?source=website&campaign=summer-2026",
      productId: "product-stable-id",
    });
  });
  ...
  it("preserves an /app prefix exactly once", () => {
    ...
    assert.equal(result?.href, "https://tenant.example/app/purchase/product/product-id");
    assert.doesNotMatch(result!.href, /\/app\/app\//);
  });

  it("returns null without APP and has no PUBLIC fallback", () => {
    assert.equal(buildProductPurchaseHandoff({ productId: "product-id", appEndpoint: null }), null);
  });
  ...
  it("rejects unsafe attribution, identifiers, and non-APP endpoints", () => {
    ...
    assert.throws(() => buildProductPurchaseHandoff({ productId: "../other", appEndpoint: endpoint("https://app.tenant.example") }));
    const publicEndpoint = { ...endpoint("https://app.tenant.example")!, role: "PUBLIC" as const };
    assert.throws(() => buildProductPurchaseHandoff({ productId: "id", appEndpoint: publicEndpoint }), /APP endpoint/);
  });
  ...
  it("changes the public representation ETag when the APP target changes", () => {
    ...
    const first = publicCatalogResponse(request, { purchase: { href: "https://app.tenant.example/purchase/product/id", productId: "id" } });
    const second = publicCatalogResponse(request, { purchase: { href: "https://tenant.example/app/purchase/product/id", productId: "id" } });
